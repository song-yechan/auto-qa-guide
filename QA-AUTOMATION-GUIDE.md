...
4. URL 입력하면 브라우저 열림 → Google 로그인 → Inspector 창 닫기
5. 로그인 완료 후 테스트 진행
...
- 네, 앱 설정 필요
```
→ "네" 선택 시: **반드시 대시보드에 표기된 앱 이름**을 입력 (앱 목록에서 해당 이름으로 찾아 진입)
→ **앱 접근 자동 검증**: 테스트 코드에서 해당 앱에 정확히 접근했는지 자동으로 확인하고, 다른 앱이면 테스트 실패 처리
...
└─────────────────────────────────────┘
                 ↓
Phase 3.5: 앱 접근 확인 (앱 설정한 경우만) [브라우저 표시]
┌─────────────────────────────────────┐
│ 8. 앱 접근 테스트 실행 (--headed)     │
│    → 브라우저가 열리고 앱까지 이동     │
│ 9. 결과 처리:                        │
│    - 앱 없음 → 알림 후 이름 재입력     │
│    - 앱 있음 → 화면 확인 요청         │
...
└─────────────────────────────────────┘
                 ↓
Phase 4: 실행 [백그라운드 - 브라우저 표시 안 함]
┌─────────────────────────────────────┐
│ 11. 페이지 구조 파악 (codegen, 필요시) │
│ 12. 테스트 코드 작성 (.spec.ts)       │
│ 13. Playwright 테스트 실행 (headless) │
│     → 브라우저 창 없이 백그라운드 실행  │
└─────────────────────────────────────┘
                 ↓
Phase 5: 산출물 저장
┌─────────────────────────────────────┐
│ 14. report/, results/ 폴더 정리       │
│ 15. 결과 리포트 작성 (.md)            │
└─────────────────────────────────────┘
                 ↓
Phase 6: 검증
┌─────────────────────────────────────┐
│ 16. 테스트 리포트 브라우저에서 오픈     │
│ 17. 객관식 선택지 제시                │
│     - 테스트 완료 → 종료              │
│     - UI 모드로 추가 확인 → 실행       │
└─────────────────────────────────────┘
...
## 별첨 A: QA 테스트 요청 템플릿 (권장)

아래 템플릿을 사용하면 자동화에 필요한 정보를 구조화해서 전달할 수 있습니다.
...
## 1. 기본 정보
- **기능명**: [예: 트래킹 링크 삭제]
- **Vercel URL**: [예: https://abrge-app-git-feature-xxx-ab180.vercel.app]
- **앱 이름** (대시보드 표기): [예: bookscribe]
- **앱 모드**: [운영 / 개발]

## 2. 테스트 케이스

| # | 페이지/화면 | 사전조건 | 동작 | 예상 결과 | 비고 |
|---|------------|---------|------|----------|------|
| 1 | [페이지명] | [조건] | [수행할 동작] | [확인할 결과] | |
| 2 | | | | | |
...
## 5. 특이사항 (선택)
- **다단계 접근**: [예: 삭제 버튼은 행 선택 → 수정 버튼 클릭 후 노출]
- **확인 모달**: [예: AB180 내부 확인 모달에서 앱 이름 입력 필요]
- **동적 데이터**: [예: 테스트 전 트래킹 링크 생성 필요]
```
...
## 1. 기본 정보
- **기능명**: 트래킹 링크 삭제
- **Vercel URL**: https://abrge-app-git-feature-abrfe-2100-ab180.vercel.app
- **앱 이름**: bookscribe
- **앱 모드**: 운영
...
| # | 페이지 | 사전조건 | 동작 | 예상 결과 |
|---|--------|---------|------|----------|
| 1 | 대시보드 | 운영 모드 | 접근 | 전체 삭제 버튼 미노출 |
| 2 | 삭제 모달 | - | 최초 로드 | 삭제 버튼 비활성화 |
| 3 | 삭제 모달 | - | 잘못된 값 입력 | 버튼 비활성화 유지 |
| 4 | 삭제 모달 | - | 올바른 값 입력 | 버튼 활성화 |
| 5 | 삭제 모달 | 버튼 활성화 | 삭제 클릭 | Snackbar 노출 |

## 3. UI 플로우
앱 목록 → bookscribe → 트래킹 링크 → 트래킹 링크 관리 → 행 선택 → 수정 → 삭제

## 4. 특이사항
- 삭제 버튼은 행 선택 후 수정 버튼 클릭해야 노출됨
- AB180 확인 모달에서 앱 이름(bookscribe) 입력 필요
- 최종 확인 모달에서 한 번 더 삭제 클릭 필요
```
...
테스트 케이스가 불명확할 때, 아래 프롬프트를 Claude나 ChatGPT에 입력하면 자동화에 적합한 형식으로 변환됩니다.
